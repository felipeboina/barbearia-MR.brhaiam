-- ============================================================================
-- Porta as funcionalidades novas do protótipo mais recente:
-- Atendimento Avulso, Relacionamento (fidelidade+aniversariantes+inativos
-- unificados, com toggles liga/desliga), Mensagens (templates configuráveis),
-- horário individual por barbeiro, custo de produto (COGS automático).
-- Rode depois do 0001_init.sql e 0002_complete_appointment.sql.
-- ============================================================================
...
alter table public.tenants
  ...
  add column if not exists message_sender_number text not null default '',
...
-- ----------------------------------------------------------------------------
-- complete_appointment: recriada para (1) respeitar loyalty_enabled ao
-- conceder ponto de fidelidade e (2) lançar despesa automática de custo de
-- produto (COGS) para os produtos vendidos junto do agendamento, igual ao
-- que já acontece na venda avulsa pelo Estoque.
-- ----------------------------------------------------------------------------
...
begin
  ...
  if jsonb_array_length(coalesce(v_appt.products, '[]'::jsonb)) > 0 then
    for v_item in select * from jsonb_array_elements(v_appt.products) loop
      ...
      -- custo do produto (COGS) — só lança despesa se o produto tiver custo cadastrado
