-- ============================================================================
-- complete_appointment: a operação mais crítica do sistema (concluir um
-- atendimento). Precisa ser atômica — status do agendamento, transação
-- financeira + comissão, transações de produtos vendidos, baixa de estoque,
-- e atualização do cliente (visitas, gasto total, fidelidade, indicação,
-- consumo de corte do plano) tudo ou nada. Portado de handleComplete
-- (barbearia-app.jsx, linhas 3756-3813).
--
-- security invoker (padrão) => roda com o papel de quem chamou, então as
-- políticas de RLS de cada tabela continuam valendo normalmente aqui dentro.
-- Rode este script depois do 0001_init.sql.
-- ============================================================================

alter table public.clients add column if not exists redemptions_count int not null default 0;
